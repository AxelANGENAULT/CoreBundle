...
class AriiSQL
{
    ...
    public function Select( $Columns = array(), $option='' ) {   
        ...
        foreach ($Columns as $c) {
            if (($p = strpos($c,' as '))>0) {
                array_push($Select,$this->Column(trim(substr($c,0,$p+1)),trim(substr($c,$p+4))));
            }
            else {
                array_push($Select,$this->Column(trim($c)));
            }
        }
...
// permission
               case '{job_name}':
                   if (isset($Fields['job_name'])) {
                    array_push($Where, '( '.$this->Column($Fields['{job_name}'])." like '".$this->Column($Fields['job_name'])."' )" );                   
                   }
...
/*            if ($this->Status['RUNNING']>0) {
                array_push($Filter, '( isnull('.$Fields['end_time'].') )');
            }
            elseif (isset($Fields['stopped'])) {
                if ($this->Status['STOPPED']>0) {
                    array_push($Filter, '( '.$Fields['stopped'].'=1 )');
                    unset($Fields['stopped']);
                }                
            }
            else {
 */
...
/*        if (isset($Fields['end_time']))
            array_push($Filter, '( '.$Fields['end_time'].' > "'.$this->ref_date.'" - INTERVAL '.($this->ref_past*-1).' HOUR )' );
*/
        //           }
//        } 
/*        if (isset($Fields['start_time'])) {
            array_push($Filter, '( '.$Fields['start_time'].' > "'.$this->ref_date.'" - INTERVAL '.($this->ref_past*-1).' HOUR )' );
        }
*/
        /*        if (isset($Fields['stopped'])) {
            if ($this->Status['STOPPED']>0) {
                array_push($Filter, '( '.$Fields['stopped'].'=1 )');
            }                
        }
 */
   /*     if (isset($Fields['error'])) {
            $Done = array();
            if ($this->Status['FAILURE']>0) {
                array_push($Done, '( '.$Fields['error'].'>0 )');
            }
            if ($this->Status['SUCCESS']>0) {
                array_push($Done, '( '.$Fields['error'].'=0 )');
            }
            if (count($Done)>0) {
                array_push($Filter, '( '.implode(' or ',$Done).' )');
            }
        }
